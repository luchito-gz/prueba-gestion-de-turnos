...
export default function Clientes(){
  ...
  const fetchClientes=async token=>{
    ...
    try{
      const res=await fetch(`${API_BASE}/api/clientes`,{headers:{Authorization:`Bearer ${token}`}});
  ...
  const handleCreate=async e=>{
    ...
    try{
      const res=await fetch(`${API_BASE}/api/clientes`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({nombre:form.nombre,telefono:form.telefono,email:form.email||null})});
      if(res.status===401){localStorage.removeItem('token');navigate('/login');return;}
      ...
      setSuccess('Cliente creado correctamente.');setForm(EMPTY_FORM);fetchClientes(token);
  ...
  const handleDelete=async id=>{
    ...
    try{
      const res=await fetch(`${API_BASE}/api/clientes/${id}`,{method:'DELETE',headers:{Authorization:`Bearer ${token}`}});
      if(res.status===401){localStorage.removeItem('token');navigate('/login');return;}
      if(!res.ok){const d=await res.json().catch(()=>({}));throw new Error(d.detail||d.message||`Error ${res.status}`);}
      setSuccess('Cliente eliminado.');setClientes(prev=>prev.filter(c=>c.id!==id));
    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:`radial-gradient(ellipse at 75% 40%,rgba(88,28,235,0.3) 0%,transparent 55%),radial-gradient(ellipse at 25% 70%,rgba(37,99,235,0.15) 0%,transparent 50%),#050510`,fontFamily:DS.font,padding:"24px 28px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:20}}>
          ...
          <h1 style={{margin:0,fontSize:18,fontWeight:700,color:DS.text,letterSpacing:.5}}>Clientes</h1>
        </div>
        ...
        <div style={{background:DS.cardBg,border:`1px solid ${DS.borderCard}`,borderRadius:14,overflow:"hidden",backdropFilter:"blur(20px)",boxShadow:"0 20px 50px rgba(0,0,0,0.4)",marginBottom:16}}>
          {loading?(
            <div style={{textAlign:"center",padding:"40px 0",color:DS.textSec,display:"flex",flexDirection:"column",alignItems:"center",gap:10}}>
              ...
              <span style={{fontSize:13}}>Cargando clientes...</span>
            </div>
          ):clientes.length===0?(
            <div style={{textAlign:"center",padding:"40px 0",color:DS.textDis,fontSize:13}}>No hay clientes registrados todavía.</div>
          ):(
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead>
                <tr style={{background:"rgba(15,23,42,0.8)"}}>
                  {["Nombre","Teléfono","Email","Acción"].map(h=>(
              ...
              <tbody>
                {clientes.map((c,i)=>(
                  <tr key={c.id??i} style={{borderBottom:`1px solid ${DS.borderCard}`,background:i%2===0?"transparent":"rgba(255,255,255,0.015)"}}>
                    <td style={{padding:"10px 16px",fontSize:13,color:DS.text,fontFamily:DS.font,fontWeight:500}}>{c.nombre||'—'}</td>
                    <td style={{padding:"10px 16px",fontSize:13,color:DS.textSec,fontFamily:DS.font}}>{c.telefono||'—'}</td>
                    <td style={{padding:"10px 16px",fontSize:13,color:DS.textSec,fontFamily:DS.font}}>{c.email||'—'}</td>
                    <td style={{padding:"10px 16px"}}><DangerBtn onClick={()=>handleDelete(c.id)} disabled={deletingId===c.id}>{deletingId===c.id?'Eliminando...':'Eliminar'}</DangerBtn></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Formulario */}
        <div style={{background:DS.cardBg,border:`1px solid ${DS.borderCard}`,borderRadius:14,padding:"20px 24px",backdropFilter:"blur(20px)",boxShadow:"0 20px 50px rgba(0,0,0,0.4)",maxWidth:600,position:"relative"}}>
          <BracketTL/><BracketBR/>
          <div style={{fontSize:12,fontWeight:700,color:DS.cyan,textTransform:"uppercase",letterSpacing:2,marginBottom:16,fontFamily:DS.font}}>+ Agregar cliente</div>
          <form onSubmit={handleCreate}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
              <div><Label required>Nombre</Label><DSInput type="text" name="nombre" placeholder="María López" value={form.nombre} onChange={e=>setForm({...form,nombre:e.target.value})} required/></div>
              <div><Label required>Teléfono</Label><DSInput type="text" name="telefono" placeholder="11-9876-5432" value={form.telefono} onChange={e=>setForm({...form,telefono:e.target.value})} required/></div>
              <div style={{gridColumn:"1/-1"}}><Label>Email</Label><DSInput type="email" name="email" placeholder="cliente@email.com" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></div>
            </div>
            <CTA loading={saving} disabled={saving}>{saving?"Guardando...":"Crear cliente"}</CTA>
          </form>
        </div>
      </div>
    </>
